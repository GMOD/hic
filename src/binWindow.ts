 ...
 * chromosome-aligned queries juicebox issues. A genome browser queries
 * arbitrary viewport windows, where `start % binsize !== 0` essentially always
 * holds, and the raw quotient then (a) drops the bin straddling the region's
 * start — a missing column at the left edge of every block, jittering as the
 * user pans — while keeping the one straddling the end, and (b) selects
 * *nothing at all* for a region narrower than one bin, which is reachable by
 * locking a coarse binsize while zoomed in.
 *
