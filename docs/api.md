...
Exactly one of `filehandle`, `path` or `reader` is required. Nothing is read
until the first call; the header parse is shared by every caller that arrives
while it is in flight, and retried if it fails.

`nvi` is optional and only matters for pre-v9 files, which do not record where
their normalization vector index lives. Without it that position is discovered
by walking the expected-value vectors — a chain of small sequential reads, paid
once per file. A v9 file records the position in its header and skips the walk.
...
Every bin **overlapping** the requested region is returned, including the ones
straddling either edge, so a region narrower than one bin still returns that
bin.

`appliedNormalization` is the scheme that was actually applied. A file stores
...
`transposed` says the query was swapped, because a `.hic` stores only the
...
Chromosome names are matched through an alias table built from the file's own
names, so `'1'` and `'chr1'` both work whichever the file uses.

Throws an error beginning with `NO_DATA_FOR_RESOLUTION` (exported) if the
region pair has a matrix but no data at `binsize`. A caller fetching many pairs
at once will usually want to drop that pair rather than fail the whole fetch,
which is why the constant is exported rather than hand-copied.
...
`['NONE', 'VC', 'VC_SQRT', 'KR', 'SCALE']`. Computed as a side effect of
loading the normalization vector index, so the first call may read.
...
this parser uses internally. The constructor calls it for you; it is exported
for wrapping or instrumenting reads.

A `Reader` must answer a read past end-of-file **short**, not throw. The header
walk reads speculatively past the end: the master-index size is an estimate,
and a zero-length answer to the norm-vector index probe is how a file carrying
no normalization data is recognized. Every `generic-filehandle2` handle behaves
this way, remote 416s included.
