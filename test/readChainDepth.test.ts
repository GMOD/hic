 ...
 */
function batchingReader(inner: Reader) {
  let pending: (() => void)[] = []
  let scheduled = false
...
test.each([
  [
    ...
    // private, and the point is the chain of reads it issues
    (hic: HicFile) =>
      (hic as unknown as PrivateChains).getNormVectors(
        'KR',
        region('3'),
        region('3'),
        'BP',
