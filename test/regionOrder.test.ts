...
import { TEST_HIC } from './testFile.ts'
...
// the caller can un-swap. A multi-region view is what makes the swap reachable
...
function open() {
  return new HicFile({ path: TEST_HIC })
}
...
test('an overlapping same-chr pair is transposed when displayed right-to-left', async () => {
  ...
  // the caller un-swaps by differs. Comparing `region1.start` against
